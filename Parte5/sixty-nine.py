...
class Aplicacion:
  ...
  def agregar_componentes(self):
    ...
    self.suma = 0
    self.labelRespuesta = ttk.Label(self.labelframe1, text=self.suma)
    self.labelRespuesta.grid(column=1, row=2)
    self.labelRespuesta.configure(foreground="blue")
  ...
  def sumar(self):
    if self.dato1.get() == "" or self.dato2.get() == "":
      mb.showerror("Cuidado", "No puede dejar los cuadrados de entrada de números vacios")
    else:
      self.suma = int(self.dato1.get()) + int(self.dato2.get())
      # self.ventana1.title("La suma es "+str(suma))
      self.labelRespuesta.config(text=self.suma)
